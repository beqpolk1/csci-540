...
public class TestDBQueries {
    ...
    //use a simple nested loop algorithm
    ...
    //select all tuples/records from the person relation and join to include food data
    //use a simple nested loop algorithm
    public static void nestedLoopJoin2(Environment env) {
        checkEnv(env);
        env.getDisk().reset();
        Long ops = 0L;

        String table1 = "person", table2 = "food";
        ...
        String joinField1 = "fav_food", joinField2 = "id";
        Integer joinField1Index = table1Cols.indexOf(joinField1), joinField2Index =table2Cols.indexOf(joinField2);
    ...
    //select all tuples/records from the person relation and join to include hair_color data
    //use the block nested loop algorithm
    public static void blockNestedLoopJoin(Environment env) {
        checkEnv(env);
        env.getDisk().reset();
        Long ops = 0L;
        ...
        Queue<QueryAction> queryActions = new LinkedList<>();
        queryActions.add(new BlockNestedLoopJoin(env.getTableMeta(table1), env.getTableMeta(table2)));
        QueryCost estCost = CostEstimator.estimateCost(queryActions);
        ...
        System.out.println("Done joining " + table1 + " and " + table2 + " with block-nested algo");
    ...
    public static void indexedNestedLoopJoin(Environment env) {
        checkEnv(env);
        env.getDisk().reset();
        Long ops = 0L;
        ...
        Queue<QueryAction> queryActions = new LinkedList<>();
        queryActions.add(new IndexNestedLoopJoin(env.getTableMeta(table1), env.getTableMeta(table2), 4));
        QueryCost estCost = CostEstimator.estimateCost(queryActions);
        ...
        while (curBlock1 != null) {
            ...
            for (Tuple curTuple1 : tab1Block) {
                BNode curNode = env.getIndex(table2, joinField2).getStartNode((Comparable) curTuple1.getField(table1Cols.indexOf(joinField1)));
                ops += (long) curNode.getSearchOps();

                for (int i = 0; i < curNode.getSize() - 1; i++) {
                    ops++;

                    if (curNode.getVal(i).compareTo(curTuple1.getField(table1Cols.indexOf(joinField1))) == 0) {
                        ArrayList<String> addr = curNode.getAddr(i);
                        ...
                        for (String curAddr : addr) {
                            ...
                            ExternalMem curBlock2 = env.getBlock(blockAddr);
                            List<Tuple> tab2Block = (List<Tuple>) DataReader.readMem(curBlock2, env.getDisk(), col2Types);
                            Tuple curTuple2 = tab2Block.get(Integer.parseInt(recAddr) - 1);

                            List<Object> joined = (List) curTuple1.getAllFields();
                            for (int j = 0; j <= table2Cols.size() - 1; j++) {
                                if (!(table2Cols.get(j).equals("id"))) joined.add(curTuple2.getField(j));
                            }
                            joinedContents.add(new ListTuple(joined));
                        }
